...
lag_df$period[lag_df$Date>'2019-10-01']<-'controlPeriod'
lag_df$period<-droplevels(lag_df$period)
levels(lag_df$period)<-c(0,1)
#run cross basis
cb<-crossbasis(lag_df$period,lag=44,argvar=list(fun="strata",breaks=1),
  arglag=list(fun="strata",breaks=c(1,14)))
#fill na with 0
cb[is.na(cb)]<-0
