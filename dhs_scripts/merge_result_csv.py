...
import glob, os
first_dir= ".\\"#r"Z:\Balaji\Analysis_out_IPOP\13082020_final1_manu\SVI_adjusted_res"
req_files=glob.glob("./**/*_reg.csv", recursive=True)
...
for file in req_files:
    ...
    Dis_cat=os.path.basename(file).replace("_reg.csv","")
    df['outcome']=Dis_cat
    df['iteration']=os.path.dirname(file).replace('.\\', "")
        
    merge_df=pd.concat([merge_df,df],axis=0)
    
merge_df.columns=['covar', 'RR', 'P', 'conf25', 'conf95','model', 'outcome', 'iteration']
merge_df['covar']=merge_df['covar'].str.replace("\[T.",'_').str.replace('\]','')
#merge_df['folder']='SVI_Cat_T4'

#% outupt
merge_df.to_excel(first_dir+r'\merged_flood_floodcats.xlsx',index=False)  