...
#post flood 1
all_df<-read_excel("merged_all.xlsx",sheet='Results_formated',range="A25:M42")
#post flood 2
#all_df<-read_excel("merged_all.xlsx",sheet='Results_formated',range="A47:M64")

colnames(all_df)<-c('Outcome',rep(c('RR','conf25','conf95'),4))
flood_df<-all_df[-1,1:4]
flood_df<-rbind(flood_df,all_df[-1,c(1,5,6,7)])
flood_df<-rbind(flood_df,all_df[-1,c(1,8,9,10)])
flood_df<-rbind(flood_df,all_df[-1,c(1,11,12,13)])
flood_df$Period<-rep(c('SVI 1','SVI 2','SVI 3','SVI 4'),each=length(unique(flood_df$Outcome)))

loutcomes<-list()
loutcomes[[1]]<-c("Bite-Insect","CO Poisoning","Dehydration","Drowning","Heat Related Illness","Hypothermia")
loutcomes[[2]]<-c("All","Mortality","Pregnency Complications","Medication_Refill","Dialysis")
loutcomes[[3]]<-c("Intestinal_infectious_diseases","ARI","Chest Pain/Palpitation","Asthma")
...
(p <- ggplot(flood_df_sub, aes(y = RR, x = xindex ,color=factor(Period))) + 
    ...
    geom_point(size = 6,shape="_") +
    geom_hline(aes(yintercept = 1), size = .25, linetype = "dashed")+
    geom_vline(data=flood_df_sub[seq(4,length(flood_df_sub$xindex)-1,4),],aes(xintercept=xindex+.5),color='gray50',size=.4,linetype = "dotted")+
    scale_x_continuous(breaks=flood_df_sub$xindex[seq(2,length(flood_df_sub$xindex),4)]+.5,
                       labels=flood_df_sub$Outcome[seq(2,length(flood_df_sub$Outcome),4)])+
    scale_y_log10()+
    theme_bw()+
    theme(panel.grid.minor = element_blank(),panel.grid.major.x = element_blank(),axis.ticks.length.x=unit(-0.1, "cm"),
          axis.text.x = element_text(vjust = 7),legend.title = element_blank(),
