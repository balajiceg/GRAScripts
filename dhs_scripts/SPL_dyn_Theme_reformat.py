...
dyn_svi_raw=pd.read_csv('Z:/Balaji/Dynamic_SVI/spl3_theme_raw.csv')

county_to_filter=pd.read_csv('Z:/Balaji/counties_inun.csv').GEOID.to_list()
dyn_svi=dyn_svi_raw[(dyn_svi_raw.FIPS//1000000).isin(county_to_filter)].copy()
...
for i in range(7):
    ...
    df=pd.DataFrame({'FIPS':dyn_svi.FIPS,'Day_of_week':i,'Theme_3':avg})
    outdf=pd.concat([outdf,df])
    
outdf.to_csv('Z:/Balaji/Dynamic_SVI/SPL_Theme3.csv')
