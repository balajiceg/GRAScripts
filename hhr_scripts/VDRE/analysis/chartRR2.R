...
setwd("D:\\NASAProjectFiles\\Analysis_HHR\\working_files")
AXIS_Y_SIZE<-12
LEGEND_SIZE<-12
Y_TITLE_SIZE<-14
...
#read excel
allDf<-read.xlsx("D:\\NASAProjectFiles\\Analysis_HHR\\working_files\\mergedOutputs.xlsx")
#subet base model alone
allDf<-filter(allDf,model=='baseModel')
...
reqRes<-subset(reqRes,outcome %in% c("Illness", "Injury", "Concentrate", "Headaches", "RunnyNose", "ShortBreath", "SkinRash", "Hospital"))
...
reqRes$term<-gsub('Contact_Water','z1',reqRes$term)
...
reqRes$exposure<-gsub('Contact_Water','Reported \nwater contact',reqRes$exposure)
...
reqRes1<-subset(reqRes,!outcome %in% c("Headaches", "RunnyNose", "Shortness of breath", 'Hospital'))
#draw plot
...
x1<-ggplot(reqRes2, aes(y = estimate, x = exposure,color=factor(term))) + facet_wrap(~ outcome,nrow=1,ncol = 4)+
  geom_errorbar(aes(ymax = conf.high, ymin = conf.low), size = LINE_WIDTH, width =
                  ERROR_BAR_TOP,position = position_dodge(width = DODGE_WIDTH)) +
  geom_point(size = POINT_WIDTH,position = position_dodge(width = DODGE_WIDTH)) +
  #scale_shape_manual(values=c(15,17,16,18,8,16,17,15,18,8,9))+
...
comb<-ggarrange(x,x1)
comb
#ggsave("confintGraph.png",plot =comb ,width = unit(11.5,'cm'),height=unit(8,'cm'))
...
allDf<-read.xlsx("D:\\NASAProjectFiles\\Analysis_HHR\\working_files\\mergedOutputs.xlsx")
#subet base model alone
allDf<-filter(allDf,model=='sviInteractionModel')
...
reqRes<-filter(allDf,grepl(':',allDf$term)) %>% filter(exposure %in% req_terms, grepl('cat_SVI',otherCom)) 

reqRes<-subset(reqRes,outcome %in% c("Illness", "Injury", "Concentrate", "Headaches", "RunnyNose", "ShortBreath", "SkinRash"))

#futher sutsub overall SVI alone
reqRes<-filter(reqRes,otherCom == 'interaction term cat_SVI')

#create seperate variable for interaction term
reqRes$inter<-str_split(reqRes$term,'SVI',simplify = T)[,2]
...
reqRes$inter<-factor(reqRes$inter,levels = c("midLow" , "midHigh" ,"highest"))

reqRes1<-subset(reqRes,!outcome %in% c("Headaches", "RunnyNose", "Shortness of breath"))
#draw plot
x<-ggplot(reqRes1, aes(y = estimate, x = exposure,color=factor(inter))) + facet_wrap(~ outcome,nrow=1)+
  geom_errorbar(aes(ymax = conf.high, ymin = conf.low), size = LINE_WIDTH, width =
                  ERROR_BAR_TOP,position = position_dodge(width = DODGE_WIDTH)) +
  geom_point(size = POINT_WIDTH,position = position_dodge(width = DODGE_WIDTH)) +
  #scale_shape_manual(values=c(15,17,16,18,8,16,17,15,18,8,9))+
  scale_color_manual(values=c( "#009E73","#0072B2","#D55E00","#CC79A7","#E69F00","#999999","#000000","#E69F00","#009E73","#F0E442"))+
  geom_hline(aes(yintercept = 1), size = DASH_WIDTH, linetype = "dashed")+
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", function(x) round(10^x,1)))+
  theme_bw() +   ylab("Rate ratio") + xlab("") +
...
reqRes2<-subset(reqRes,(outcome %in% c("Headaches", "RunnyNose", "Shortness of breath")))
x1<-ggplot(reqRes2, aes(y = estimate, x = exposure,color=factor(inter))) + facet_wrap(~ outcome,nrow=1)+
  geom_errorbar(aes(ymax = conf.high, ymin = conf.low), size = LINE_WIDTH, width =
                  ERROR_BAR_TOP,position = position_dodge(width = DODGE_WIDTH)) +
  geom_point(size = POINT_WIDTH,position = position_dodge(width = DODGE_WIDTH)) +
  #scale_shape_manual(values=c(15,17,16,18,8,16,17,15,18,8,9))+
...
comb<-ggarrange(x,x1,nrow = 2)
comb
ggsave("confintGraphSviInter.png",plot =comb ,width = unit(9.4,'cm'),height=unit(8,'cm'))
